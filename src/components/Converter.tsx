...
import { ALL_FORMATS } from '../lib/converter/CommonFormats';
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (isMedia && currentFfmpeg) {
          ...
          blob = await handler.doConvert(file, targetFormat);
              ...
              <div className="flex items-center gap-3 overflow-hidden">
                ...
                <div className="truncate">
                  ...
                  <p className={`text-xs ${theme === 'dark' ? 'text-[#888]' : 'text-[#666]'}`}>
                    {files.length === 1 ? (files[0].size / 1024 / 1024).toFixed(2) + ' MB' : `${(files.reduce((acc, f) => acc + f.size, 0) / 1024 / 1024).toFixed(2)} MB total`}
                  </p>
                </div>
              </div>
              <button 
