...
import { Settings, X, History, FileType } from 'lucide-react';
        ...
        <div className="absolute top-5 right-6 z-20 flex gap-3">
          {loading ? (
            <div className="flex gap-3">
              <div className={`w-[84px] h-[38px] rounded-full animate-pulse ${theme === 'dark' ? 'bg-white/10' : 'bg-black/10'}`}></div>
              <div className={`w-[84px] h-[38px] rounded-full animate-pulse ${theme === 'dark' ? 'bg-white/10' : 'bg-black/10'}`}></div>
            </div>
          ) : !user ? (
            <>
            ...
            <>
              <button onClick={() => setModals({...modals, history: true})} className={`px-4 py-2 rounded-full text-sm border flex items-center gap-2 transition-all ${theme === 'dark' ? 'border-[#444] text-[#aaa] hover:border-[#777] hover:text-white bg-transparent' : 'border-[#ccc] text-[#555] hover:border-[#999] hover:text-black bg-transparent'}`}>
                <History size={16} /> History
              </button>
              <button onClick={handleLogout} className={`px-4 py-2 rounded-full text-sm border transition-all ${theme === 'dark' ? 'border-[#444] text-[#aaa] hover:border-red-500 hover:text-red-500 bg-transparent' : 'border-[#ccc] text-[#555] hover:border-red-500 hover:text-red-500 bg-transparent'}`}>Sign out</button>
            </>
          )}
        </div>
